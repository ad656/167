...
Scene* sphere_in_box() {
    ...
    //std::shared_ptr<GlossyMaterial> glossy_material = std::make_shared<GlossyMaterial>(vec3(0.5f, 0.5f, 1.0f), vec3(0.25f), 0.9f);
    ...
    std::shared_ptr<GlossyMaterial> copper_material = std::make_shared<GlossyMaterial>(
        vec3(0.72f, 0.45f, 0.20f),  // Diffuse color (reddish-orange)
        vec3(0.95f, 0.64f, 0.54f),  // Specular color (slightly brighter reddish-orange)
        0.85f                       // Shininess (controls roughness)
    );
    ...
    square_light->model = std::make_unique<Square>(vec3(0.0f, 0.0f, 0.0f), 2.0f, vec3(0.0f, -1.0f, 0.0f), light_material);
    ...
    /*
    std::unique_ptr<Node> tetrahedron = std::make_unique<Node>();
    tetrahedron->model = std::make_unique<Tetrahedron>(2.5f, copper_material);
    root_node->childnodes.push_back(std::move(tetrahedron));
    root_node->childtransforms.push_back(translate(vec3(-0.3f, -0.6f, 0.0f)));
    */
