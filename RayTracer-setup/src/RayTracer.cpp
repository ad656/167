...
Ray RayTracer::ray_thru_pixel(int i, int j) {
     ...
     * This will return a random value in the range [0,1]
     */
    float x = glm::linearRand(0.0f, 1.0f);
    float y = glm::linearRand(0.0f, 1.0f);


    float alpha = (2.0f * ((float)i + x) / (float)camera.width) - 1.0f;
    float beta = 1.0f - (2.0f * ((float)j + y) / (float)camera.height);
    ...
    float tan_fov_half = tan(camera.fovy / 2.0f);
    float aspect = (float)camera.width / (float)camera.height;
    
 
    ray.dir = normalize(-w + u * alpha * tan_fov_half * aspect + v * beta * tan_fov_half);
    return ray;
}
